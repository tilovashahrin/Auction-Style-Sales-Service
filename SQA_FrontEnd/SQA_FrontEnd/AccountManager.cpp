...
AccountManager::AccountManager() {
	...
	setItemFile("DataFiles/ItemDB.txt");
	setTransFile("DataFiles/Transout.atf");
} 
...
AccountManager::AccountManager(string _accountFile, string _itemFile, string _transFile){
	...
	setItemFile(_itemFile);
	setTransFile(_transFile);
}
...
void AccountManager::login(string _username) // 0 = Success, 1 = Already Logged In, 2 = No Account
{
	...
	if (myfile.is_open())
	{
		...
		cout << "Unable to open username file.\n";
		exit(1); //return;
	}
	cout << "ERROR: Cannot log in, username" << username << " was not found.\n";
	exit(1);
}
...
void AccountManager::logout() // 0 = Success, 1 = Already Logged Out
{
	...
	//Transaction::logout_trans(00, username, user_type, credits);
	logStatus = false;
	username = "";
	credits = 00.00;
	user_type = "";
...
void AccountManager::createUser() { 
	...
	//	infile.close();
	//	outfile.close();
	//	if (remove("DataFiles/UserDB.txt") != 0 || rename("temp.txt", "DataFiles/UserDB.txt") != 0) {
	...
	Transaction::logout_trans(01, _username, _utype, credits);
}
...
void AccountManager::deleteUser() {
	...
	cout << "\nDeletion Complete. Information saved into the daily transaction file.\n";
	Transaction::logout_trans(02, _username, user_type, credits);
}
...
string AccountManager::getUser(string _username)
{
